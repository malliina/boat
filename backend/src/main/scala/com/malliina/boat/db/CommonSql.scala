...
trait CommonSql:
  ...
  private val trackHighlights =
    sql"""select t.id, t.name, t.title, t.canonical, t.comments, t.added, t.points, t.avg_speed, t.avg_water_temp, t.distance, t.start, t.end, t.secs, t.startDate, t.startMonth, t.startYear, t.boat, top.id pointId, top.longitude, top.latitude, top.coord, top.speed, top.water_temp, top.depthm, top.depth_offsetm, top.source_time, date(top.source_time) trackDate, top.track, top.added topAdded
  ...
  val pointColumns =
    fr"p.id, p.longitude, p.latitude, p.coord, p.speed, p.water_temp, p.depthm, p.depth_offsetm, p.source_time, date(p.source_time), p.track, p.added"
